...
<table class="detailTable">
	...
	<tr align="center">
		<td height="20px;" colspan="2"><a
			href="update.bd?b_num=${bd.b_num}&pageNumber=${pageNumber}&board=${bd.b_cate}&writer=${bd.writer}">수정</a>|
			<a
			href="delete.bd?b_num=${bd.b_num}&pageNumber=${pageNumber}&board=${bd.b_cate}">삭제</a>
		</td>
	</tr>
</table>
...
<table class="detailTable">
	<c:forEach var="bd2" items="${numCommentsList}">
		<!-- 댓글 내용 표시 -->
		<tr>
			<td colspan="4">작성자: ${bd2.writer} <a style="float: right;"
				href="deleteComments.bd?b_num=${bd2.b_num}&pageNumber=${pageNumber}&ref=${bd2.ref}&originalb_num=${originalb_num}">삭제</a>

			</td>
		</tr>
		<tr>
			<td colspan="4"><c:set var="wid" value="0" /> <c:if
					test="${bd2.re_level > 0}">
		...
		<!-- 댓글 작성 폼 -->
		<tr>
			<td colspan="4"><a id="commentLink_${bd2.b_num}" href="#">대댓글</a>
				<div id="commentSection_${bd2.b_num}" style="display: none;">
					<form action="insertComments2.bd" method="post">
						...
						<input type="hidden" name="b_cate" value="${bd2.b_cate }"> <input type="hidden" name="writer" maxlength="10" value="${bd2.writer }">
						<input type="hidden" name="subject" maxlength="50" value="${bd2.subject }"> 
							<input type="hidden" name="pageNumber" value="${pageNumber }"> 
							<input type="hidden" name="ref" value="${bd2.ref }"> 
...
<div class="container">
	<form action="insertComments.bd" method="post" style="width: 40%; margin-left: 210px;">
		...
		<c:set var="listLength" value="${fn:length(numCommentsList)}" />
		<c:set var="lastIndex" value="${listLength - 1}" />
		<c:set var="lastStep" value="${numCommentsList[lastIndex].re_step}" />
		
		<c:if test="${lastStepp == 0}">
			<input type="hidden" name="re_step" value="${bd.re_step}" />
		</c:if>
		<c:if test="${lastStep > 0}">
			 <!-- bd.re_step 값이 비어있지 않을 때 해당 값 사용 -->
		        <input type="hidden" name="re_step" value="${lastStep + 1}" />
		</c:if>
		...
		<input type="hidden" name="board" value="${bd.b_cate}"> <br>
		<br>
		<div>
			<div>
			...
			<div id="commentSection">
				<table class="table">
					...
					<tr>
						<td><textarea id="commentText" style="width: 700px" rows="3"
								cols="30" name="content" placeholder="댓글을 입력하세요"></textarea> <br>
							<div>
								<input class="btn pull-right btn-success" type="submit"
									value="등록">
							</div></td>
					</tr>
				</table>
