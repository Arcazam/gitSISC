		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach var="st" items="${sb }">
					<tr>
						<td style="width: 300px; height: 50px;">${st.b_cate }</td>
						<td style="width: 200px; height: 50px;">${st.content }</td>
						<td style="width: 40px; height: 50px;">
							<a href="deleteMyComments.bd?b_num=${st.b_num}&pageNumber=1&writer=${st.writer}&id=${id}&pro_img=${loginInfo.pro_img}">삭제</a>
						</td>
					</tr>
				</c:forEach>
			</c:otherwise>
